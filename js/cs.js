// Triggered once the page has loaded:
function onPageLoad()
{
	
	/// index - slider proyectos recientes
	$('.proyectos_destacados').cycle({	fx:'fade',
																		speed:500,
																		timeout:8000,
																		prev:'#prev',
																		next:'#next'});
	//$('.proyectos_recientes').cycle('pause');
	
	
	/// index - slider frases célebres
	$('#frases').cycle({	fx:'blindY',
												sync:true,
												speed:2000,
												delay:-2000,
												// a "timeout" no se le descuenta "speed", por ello está "delay.
												// por lo tanto timeout = timeout - speed;
												timeout:10000,
												next:'#frases'});
	
	/// index - articulos squares
	setSqSize();
	
	/// todas - acciones rollOver Out menú principal
	$("#menu a").hover(function() {
				$(this).animate({ backgroundColor: "#333", color:"#EEE" }, 300);
		},function() {
		if(!$(this).hasClass('active'))
		{
			$(this).animate({ backgroundColor: "#EEE", color:"#333" }, 150);
		}
	});
	$(".btn").hover(function() {
			$(this).animate({ backgroundColor: "#CCC", color:"#333" }, 300);
		},function() {
			$(this).animate({ backgroundColor: "#333", color:"#EEE" }, 150);
	});
	
	/// proyectos - slider de proyectos destacados
	//$( "#accordion" ).accordion({autoHeight:false,navigation:true});
	
	// Shadowbox
	Shadowbox.init({flashParams:{bgcolor:"#EEEEEE"}});
	Shadowbox.setup();
...
function setSqSize()
{
	...
	for(i=1; i<=8; i++)
	{
		var wdiv = document.getElementById('square' + i);
		if(wdiv)
		{
			max = wdiv.offsetHeight > max ? wdiv.offsetHeight:max;
		}
	}
	for(i=1; i<=8; i++)
	{
...
//     http://google.co/search?q=web&aq=t&rls=org.mozilla:en-US:official&client=firefox-a
...
var Timeline = function (container, date) {
    ...
    this.renderDay = function (topRow, bottomRow, data) {
        var cssClass = "";
        cssClass += (this.isToday(data.number)) ? "hoy" : "";
...
$(document).ready(function(){
	var timeline = new Timeline("timeline", new Date("Tue Nov 01 2011"));
});